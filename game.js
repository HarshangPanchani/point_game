...
class PointGame {
            ...
            async togglePlayerActiveState(playerId) {
                ...
                const isCurrentlyInactive = player.activeInRound === false;
                const newState = !isCurrentlyInactive; // Toggle state
                ...
                if (newState === true && currentState.status === 'playing' && currentState.currentRound > 0) {
                    ...
                    if (currentPlayerId === playerId) {
                        ...
                        let nextIndex = currentState.turnIndex;
                        let failsafe = 0;
                        do {
                            nextIndex = (nextIndex + 1) % playerIds.length;
                            failsafe++;
                        } while (
                            (currentState.players[playerIds[nextIndex]] && currentState.players[playerIds[nextIndex]].activeInRound === false && playerIds[nextIndex] !== playerId) &&
            ...
            async kickPlayer(playerId) {
                ...
                if (currentPlayerId === playerId) {
                    // Find the next active player to prevent the game from getting stuck
                    let nextIndex = gameData.turnIndex;
                    do {
                        nextIndex = (nextIndex + 1) % playerIds.length;
                    } while (playerIds[nextIndex] === playerId); // Loop until we find a player that isn't the one being kicked
            ...
            renderPlayerList(gameData) {
                ...
                Object.entries(players).forEach(([pid, p]) => {
                    ...
                    const statusIndicator = `<span style="color:${p.online ? 'lightgreen' : 'gray'}">${p.online ? 'Online' : 'Offline'}</span>`;
                    const playerInfo = `<div>${p.name} ${pid === hostId ? '👑' : ''}<br><small>${statusIndicator}</small></div>`;
            ...
            render(gameData) {
                ...
                if (status === 'lobby') {
                    ...
                    Object.entries(players || {}).forEach(([pid, p]) => {
                        let li = document.createElement('li');
                        li.innerHTML = `<span>${p.name} ${pid === hostId ? '👑' : ''}</span><span style="color:${p.online ? 'lightgreen' : 'gray'}">${p.online ? 'Online' : 'Offline'}</span>`;
                        if (this.isHost && pid !== this.myId) {
                            let kickBtn = document.createElement('button');
                            kickBtn.innerText = 'Kick';
